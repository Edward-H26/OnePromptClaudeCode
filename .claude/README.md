...
- `bash scripts/doctor-workflow.sh` auto-creates or merges `.claude/settings.local.json` from that tracked example on first run.
...
- `.claude/runtime/` is repo-local ignored runtime state for safety flags and workflow artifacts.
...
- Run `bash scripts/doctor-workflow.sh` when you need a live readiness check for plugins and MCP connectivity.
