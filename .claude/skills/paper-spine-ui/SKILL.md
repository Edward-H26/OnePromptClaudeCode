---
name: paper-spine-ui
description: Launches the PaperSpine external terminal configuration UI for Claude Code. (internal /paperspine step)
---
...
`~/.claude/skills/...`). Do not run `input()`-based Python inside a
...
missing. Launch the UI as the first action when config is missing; do not infer
config from materials instead.
...
If no separate window appears (headless or sandboxed environments with no
