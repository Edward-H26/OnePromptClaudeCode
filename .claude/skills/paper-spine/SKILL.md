...
1. If configuration is missing or incomplete, run the terminal wizard from
   ...
   available. The wizard is the supported Claude Code command-line UI.
   ...
   **Hard constraint (do not skip):** when
   ...
   `launch_paperspine_ui.ps1` (resolve under `~/.claude/skills/...`) so the
...
Claude Code does not guarantee a native graphical picker for skills.
