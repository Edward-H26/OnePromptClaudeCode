...
- DO NOT simulate any audit step. DO NOT claim to have run external review. Output metadata must not claim audit-passed state.
...
## Standalone-Mode Self-Gate (v3.7.3 review round-7 F17 + round-8 F21 closure)
...
**Self-gate rule (standalone mode only).** The gate is a two-part check on the compiled report — failing EITHER part refuses emission. v3.7.3 review round-9 F22 closure (the round-7 single-part check missed bare-ref bypass).
