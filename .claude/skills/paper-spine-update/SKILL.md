...
upgrade the local Claude Code or OpenClaw PaperSpine skills while
preserving settings.
...
- update Claude Code and OpenClaw installs by default,
...
- Use `--target claude` or `--target openclaw` only when the
