...
2. **Direction questions answered** (Step 3, Direction questions)
3. **Palette confirmed** (Step 3, Palette)
4. **One mock direction approved or delegated** (Step 3, Mock approval)

You must stop at every gate. **Shape confirmation alone is NOT a green light to start coding.** It is the green light to begin the Step 3 direction questions. Compressing gates 2 through 4 because the shape brief felt complete is the dominant failure mode of this flow.
...
If the harness has **native image generation** (an `image_gen`-style tool or an equivalent image MCP), this step is mandatory. It runs as gated sub-steps, each ending with the user; do not advance past any gate on your own read of the situation:

- **Direction questions.** Ask 2-3 targeted questions about visual lane, color strategy, atmosphere, and named anchor references, tied to the shape brief. This step is required even when shape just produced a confirmed brief, because it pins palette, atmosphere, and references that shape did not. STOP and wait for answers.
- **Palette.** Generate one focused palette artifact first (typography pairing on the chosen background, primary plus accent swatches, one signature motif). Show it and ask the user to confirm or call out shifts. STOP and wait for confirmation; do not generate mocks against an unconfirmed palette.
- **Mocks.** Once the palette is confirmed, generate 1 to 3 high-fidelity north-star comps using the confirmed palette and typography. Mocks differ in structural direction (hierarchy, topology, density, composition), not in color or motif.
- **Mock approval.** Show the comps, ask what carries forward, and iterate until one direction is approved or the user explicitly delegates. STOP and wait. Summarize what to carry into code and what not to literalize from the mock.
- **Mock-fidelity inventory and assets.** Inventory the approved mock's major visible ingredients and decide an implementation for each (semantic HTML/CSS/SVG, generated raster, sourced raster, icon library, canvas/WebGL, or accepted omission). For raster ingredients, prefer the bundled `impeccable_asset_producer` subagent rather than producing inline; if you do not have permission to spawn agents, ask first. Do not load image-generation reference material into the parent thread.
