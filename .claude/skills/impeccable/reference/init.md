...
- **Both exist**: STOP and use the harness's structured question tool when one exists, otherwise ask directly in chat, to clarify what you cannot infer. Ask which file to refresh. Skip the one the user doesn't want changed.
...
STOP and use the harness's structured question tool when one exists, otherwise ask directly in chat, to clarify what you cannot infer. Ask only about what you couldn't infer from the codebase.
...
If the signal is genuinely split (e.g. a product with a big marketing landing), STOP and use the harness's structured question tool when one exists, otherwise ask directly in chat, to clarify what you cannot infer. Ask which register describes the **primary** surface. The register can be overridden per task later, but PRODUCT.md carries one default.
