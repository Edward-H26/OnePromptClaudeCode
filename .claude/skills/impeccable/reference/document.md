...
If a `DESIGN.md` already exists, **do not silently overwrite it**. Show the user the existing file and STOP and use the harness's structured question tool when one exists, otherwise ask directly in chat, to clarify what you cannot infer. whether to refresh, overwrite, or merge.
