...
3. If no design system exists yet, use the `design-consultation` skill to scaffold one before any implementation work.
4. For specific design decisions (style, color, typography), invoke `design-consultation` with a focused question or use the `figma` skill to inspect existing system components.
...
3. Use the `frontend-design` plugin when the task needs stronger visual direction.
4. For multi-screen or complex component implementations, decompose into smaller plans and use `/multi-execute` to drive each piece.
...
3. If the UI change is substantial, use `/design-review` or the `qa` skill.
